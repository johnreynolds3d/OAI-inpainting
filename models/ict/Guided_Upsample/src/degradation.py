...
def prior_degradation(
    img, clusters, prior_size
):  ## Downsample into 32x32, using origin dictionary cluster to remap the pixel intensity
    img_np = np.array(img)

    LR_img_cv2 = cv2.resize(
        img_np, (prior_size, prior_size), interpolation=cv2.INTER_AREA
    )
    ...
    primers_img = [
        np.reshape(
            np.rint(127.5 * (clusters[s] + 1.0)), [prior_size, prior_size, 3]
        ).astype(np.uint8)
        for s in primers
    ]
...
def prior_degradation_2(
    img, clusters, prior_size, K=1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--origin_data",
        type=str,
        default="/home/ziyuwan/workspace/data/ICCV_Inpainting_Test_Data/n02091635_40_60/gt_image",
        help="",
    )
    parser.add_argument(
        "--save_url",
        type=str,
        default="/home/ziyuwan/workspace/data/n02091635_40_60_degradation_1/condition_1",
        help="",
    )
