...
class ImageNetDatasetMask(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = read_img(
            selected_img_url, image_size=self.image_size, is_train=self.is_train
        )

        x = torch.from_numpy(np.array(x)).view(-1, 3)  # flatten out all pixels
        x = x[
            self.perm
        ].float()  # reshuffle pixels with any fixed permutation and -> float
        a = (
            ((x[:, None, :] - self.clusters[None, :, :]) ** 2).sum(-1).argmin(1)
        )  # cluster assignments
