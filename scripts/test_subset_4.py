...
def check_subset_4():
    ...
    if info_file.exists():
        print("✅ Found subset_4_info.csv with metadata")
    else:
        print("⚠️  subset_4_info.csv not found")
...
def main():
    ...
    for model_name, result in all_results:
        if result:
            ...
            status = "⚠️  SKIPPED (OAI models not available)"
        else:
    ...
    if failed_tests:
        ...
        print(f"\n❌ {failed_count} tests failed. Check the logs above for details.")
        sys.exit(1)
        ...
        if skipped_tests:
            ...
            print(f"⚠️  {skipped_count} tests skipped (OAI models not available).")
        print("Check the output directories for results.")
        sys.exit(0)
    else:
        print("\n⚠️  No tests were run. Check model availability.")
        sys.exit(0)
