...
def find_untracked_dir(start_path: Path = None) -> Path:
    """
...
def setup_project_data(
    ...
    """
    print(f"🚀 Setting up OAI Inpainting project data...")
    ...
    if not all(verification.values()):
        print(
            "❌ Some required components are missing. Please check your untracked data structure."
        )
        return False
    ...
    try:
        ...
        if total_size > free_space:
            print(
                f"⚠️  Warning: Estimated data size ({estimate_size(Path('/tmp'))}) may exceed available disk space ({estimate_size(Path('/tmp'))})"
            )
            if not force:
    ...
    if dry_run:
        ...
        print(
            "   You can now run the split.py script to generate train/validation/test splits."
        )
