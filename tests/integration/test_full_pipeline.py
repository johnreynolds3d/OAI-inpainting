...
def test_subset_4_pipeline_aot_gan(project_root):
    ...
    try:
        ...
        # Check that the command completed successfully (allow exit code 1)
        if result.returncode not in [0, 1]:
            msg = f"AOT-GAN test failed with unexpected exit code {result.returncode}: {result.stderr}"
            pytest.fail(msg)

        # Check that at least some variants succeeded
        celebahq_passed = "AOT-GAN CelebA-HQ   : ✅ PASSED" in result.stdout
        places2_passed = "AOT-GAN Places2     : ✅ PASSED" in result.stdout
        if not (celebahq_passed or places2_passed):
...
def test_subset_4_pipeline_ict(project_root):
    ...
    try:
        ...
        # Check that the command completed successfully (allow exit code 1)
        if result.returncode not in [0, 1]:
            msg = f"ICT test failed with unexpected exit code {result.returncode}: {result.stderr}"
            pytest.fail(msg)

        # Check that at least some variants succeeded
        ffhq_passed = "ICT FFHQ            : ✅ PASSED" in result.stdout
        places2_passed = "ICT Places2_Nature  : ✅ PASSED" in result.stdout
        if not (ffhq_passed or places2_passed):
