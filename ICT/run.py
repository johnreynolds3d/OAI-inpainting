...
if __name__ == "__main__":
    ...
    if opts.ImageNet:
        stage_2_command = (
            "CUDA_VISIBLE_DEVICES=0,1 python test.py --input "
            + opts.input_image
            + " \
                                        --mask "
            + opts.input_mask
            ...
            + " \
                                        ...
                                        --test_batch_size "
            + test_batch_size
            + " --model 2 --Generator 4 --condition_num "
            + str(opts.sample_num)
            + suffix_opt
        )
    elif opts.FFHQ:
        stage_2_command = (
            "CUDA_VISIBLE_DEVICES=0 python test.py --input "
            + opts.input_image
            + " \
                                        --mask "
            + opts.input_mask
